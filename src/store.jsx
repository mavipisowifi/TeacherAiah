...
// Core and Applied are "all-strands" categories: taken by every SHS section of
// the grade. Any other id is a specific strand only its own sections take.
export function isCrossStrand(id) {
  const x = id || CORE_STRAND_ID
  return x === CORE_STRAND_ID || x === APPLIED_STRAND_ID
}

// The two SHS semesters. A section stores a separate program for each.
...
// Short strand badge ("STEM", "Core", "Applied", …).
...
// Full strand name ("Core (all strands)", "Applied (all strands)", …).
...
// Short semester label ("1st Sem" / "2nd Sem"); '' for a non-semestered class.
...
// The schedulable "parts" of a classroom. A Senior High section stores a
// separate program per semester under `schedule.programs` ({ '1': {...}, '2':
// {...} }, each { timeSlots, grid }); every other class keeps a single top-level
// timeSlots/grid and no semester. This helper flattens either shape into a list
// of { semester, timeSlots, grid } so every reader (conflicts, individual grids,
// load counts, the generator's booking pass) can walk both with one code path.
// A non-semestered class comes back as a single part with semester '' — so all
// non-SHS behavior is unchanged.
...
// Two class times can only clash if they share a semester. A blank semester ''
// means "all year" (K–10), so it is compatible with either SHS semester.
export function semesterCompatible(a, b) {
  return a === '' || b === '' || a === b
}

// The single program (timeSlots + grid) a class shows for a given semester. For
// a semestered SHS section, the matching semester's program; otherwise (or when
// no semester is requested) the all-year / first program.
...
// clock-time strings) so a shared teacher is caught even when the two grades run
// different bell schedules.
//
// Senior High sections carry a program per semester; a teacher can only clash
// with themselves WITHIN a semester (or against an all-year K–10 class, whose
// blank semester overlaps both). So when any semestered section exists we sweep
// each semester bucket separately; an all-year class is checked in both buckets
// and its all-year-vs-all-year clusters are de-duplicated. With no semestered
// sections there is a single '' bucket, making the result byte-identical to the
// pre-semester behavior.
export function computeConflicts(schedules) {
  const list = schedules || []
  const hasSemesters = list.some((s) => scheduleParts(s).some((p) => p.semester !== ''))
  const buckets = hasSemesters ? ['1', '2'] : ['']
...
// Whether any classroom stores a per-semester program (i.e. Senior High is in
// use). Drives whether the UI shows a 1st/2nd-semester toggle at all — a school
// with only K–10 never sees it and behaves exactly as before.
...
// Sums across every schedulable part, so a Senior High teacher's two-semester
// load is counted; a non-semestered class contributes its single grid as before.
...
// both semester programs for a Senior High section; a single-grid class is
// unchanged.
...
// semester program; unchanged for non-semestered classes.
...
// The subjects one specific CLASS takes. For a non-SHS grade this is just the
// grade's subjects (strand/semester ignored) — identical to subjectsForGrade.
// For a Senior High class it is the subjects whose strand is a cross-strand
// category (Core or Applied) OR matches the section's strand, filtered to the
// section's semester (a subject with no semester runs in both). This is the
// single source of truth for "what does this SHS section study this semester".
...
// Senior High strand + semester (subjectsForClass); without it, to the whole
// grade (subjectsForGrade) exactly as before. Either way each subject is
// resolved to one teacher via the round-robin.
...
// The strict "every subject needs a teacher" gate, scoped to one Senior High
// class (strand + semester). For a non-SHS grade the strand/semester are ignored
// and this equals subjectsMissingTeacher.
...
export function autoScheduleGrid({ gradeSubjects, subjects: subjectsArg, otherSchedules, bell, rotation = 0, semester = '' }) {
  ...
  // section's semester matter — a 1st-sem SHS class never clashes with a 2nd-sem
  // one, while an all-year K–10 class (semester '') is booked against both. With
  // the default semester '' every part is compatible, so non-SHS generation is
  // unchanged.
...
const STORAGE_KEY = 'csm:data:v5'
const LEGACY_STORAGE_KEYS = ['csm:data:v4', 'csm:data:v3', 'csm:data:v2', 'csm:data:v1']
...
// patched schedule. A Senior High section keeps a program per semester
// (programs['1'|'2']); the edit lands in the matching semester's grid and the
// other semester is left byte-for-byte untouched. Every other (K–10) class has a
// single top-level grid and ignores `semester`. Used by the manual cell editor
// so add/remove works identically whichever shape the section has.
...
// Chains the v1→v2 (Grade Level Rooms), v2→v3 (global subjects) and v3→v4
// (teachers own their subjects, strands removed) migrations — all idempotent, so
// it is safe on load and on importing any backup.
export function normalizeState(raw) {
  ...
  return {
    ...
    // subject may carry a `strandId` (blank = Core / all strands) and a
    // `semester` ('1'/'2'; blank = both / not applicable). Every subject defaults
    // to the weekly cadence + no strand + no semester, so all older data reads
    // exactly as before.
    subjects: (base.subjects || []).map((s) => {
      ...
      return {
        ...
        semester: rest.semester === '1' || rest.semester === '2' ? rest.semester : '',
      }
    }),
    // Classrooms may carry a `strandId` (Senior High) and either a top-level
    // `semester` or, for SHS, a per-semester `programs` map — all preserved here.
    schedules: (base.schedules || []).map((s) => ({
      ...s,
      strandId: s.strandId || '',
  ...
  switch (action.type) {
    ...
    case 'DELETE_TEACHER':
      return {
        ...
        // Unassign the removed teacher everywhere.
        schedules: state.schedules.map((s) => ({
          ...s,
          moderatorId: s.moderatorId === action.id ? '' : s.moderatorId,
          grid: unassignTeacher(s.grid, action.id),
        })),
      }
    case 'DELETE_TEACHERS_BULK': {
      ...
      return {
        ...
        schedules: state.schedules.map((s) => ({
          ...s,
          moderatorId: ids.has(s.moderatorId) ? '' : s.moderatorId,
          grid: unassignTeachersSet(s.grid, ids),
        })),
      }
    }
    ...
    case 'DELETE_SUBJECT':
      return {
        ...
        schedules: state.schedules.map((s) => ({ ...s, grid: unassignSubject(s.grid, action.id) })),
      }
    case 'DELETE_SUBJECTS_BULK': {
      ...
      return {
        ...
        schedules: state.schedules.map((s) => ({ ...s, grid: unassignSubjectsSet(s.grid, ids) })),
      }
    }
...
export function StoreProvider({ children }) {
  ...
  const actions = useMemo(() => {
    ...
    // schedules. A Senior High section carries a strand and TWO semester programs
    // ('1' and '2') with different subjects; each semester is scheduled
    // independently into programs[sem] and, because different semesters never run
    // at the same time, they're allowed to reuse the same teachers/rooms.
    // Every other (K–10) classroom keeps its single top-level grid, exactly as
    // before. Returns { empty, patch, poolRow, placed, requested, unplaced,
    // missingTeacher } — `patch` is what to store, `poolRow` is fed forward so
    // the next section is staggered around this one.
    const buildForSchedule = (schedule, pool) => {
      ...
      const res = autoScheduleGrid({ subjects: list, otherSchedules: pool, bell, rotation })
      return {
        empty: false,
        patch: { timeSlots: res.timeSlots, grid: res.grid },
        poolRow: { ...schedule, timeSlots: res.timeSlots, grid: res.grid },
        placed: res.placed,
        requested: res.requested,
        unplaced: res.unplaced,
        missingTeacher: list.filter((s) => !s.teacherId).length,
      }
    }
    ...
    const runBatchGenerate = (ids) => {
      ...
      for (const id of order) {
        ...
        // rotation index round-robins across them, spreading the load. Senior
        // High sections build both semester programs; K–10 build a single grid.
    ...
    return {
      ...
      addSubject: (data) => {
        const subject = {
          ...
          semester: data && (data.semester === '1' || data.semester === '2') ? data.semester : '',
        }
      ...
      addSchedule: (data) => {
        ...
        // A Senior High section holds a separate program per semester (each with
        // its own bell-derived time slots and empty grid) and carries a strand.
        // Every other grade keeps a single top-level grid, seeded with the
        // grade's bell schedule so even an ungenerated grid shows the right times.
        let schedule
        if (isShsGrade(grade)) {
          const ts1 = buildAutoTimeSlots(bellForGrade(state, grade))
          const ts2 = buildAutoTimeSlots(bellForGrade(state, grade))
          schedule = {
            ...base,
            strandId: base.strandId || '',
            semester: '',
            programs: {
              1: { timeSlots: ts1, grid: emptyGrid(ts1) },
              2: { timeSlots: ts2, grid: emptyGrid(ts2) },
            },
          }
        } else {
          const timeSlots = buildAutoTimeSlots(bellForGrade(state, grade))
          schedule = {
            ...base,
            strandId: '',
            semester: '',
            timeSlots,
            grid: emptyGrid(timeSlots),
          }
        }
      ...
      duplicateSchedule: (id, overrides = {}) => {
        ...
        if (src.programs) {
          const programs = {}
          for (const sem of ['1', '2']) programs[sem] = copyProgram(src.programs[sem] || {})
      ...
      // Manual cell edits. `semester` ('1'/'2') targets a Senior High section's
      // matching program; it is ignored for a single-grid K–10 class.
      ...
      // Remove every placed subject from a class's grid (the active semester's
      // program for a Senior High section). Time rows and bands are kept.
